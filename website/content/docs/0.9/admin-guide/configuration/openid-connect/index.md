...
The current setup test is retained for the browser tab if the page reloads. If another administrator changes account mappings during review, Sambee reloads the current mapping plan in the same tested flow, discards stale edits, and requires another review. If the provider configuration changed, or the saved test expired, Sambee discards the tested flow and requires another provider test. Temporary network failures retain the flow so the result can be loaded again. If the connection is interrupted during activation, Sambee retries the same tested flow. The browser retains that exact activation request in the current tab until the server returns a definitive result, so reloading the page can recover a completed activation without repeating its changes. Provider settings, account mappings, cancellation, remapping, and Password-only recovery remain unavailable while the activation result is unresolved. After Sambee confirms activation, a failure to reload the current settings does not undo the activation; reload the page to display the active configuration.
