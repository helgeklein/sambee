...
| `frontend/src/components/Viewer/CodeMirrorFindReplacePopover.tsx` | shared compact find, match-option, and replacement controls for CodeMirror editors |
...
- `Ctrl+P`: open command mode directly
...
1. bind it through `useKeyboardShortcuts` in the feature component
1. reuse the same definition for tooltip or help-surface labels
1. test both the runtime behavior and the visible affordance
...
Markdown and text editing keep find and replace in an editor-local fixed surface near the upper-right viewport edge, below the viewer toolbar, rather than in the shared viewer toolbar. `MarkdownViewer` and `TextViewer` own the surrounding viewer state and bind `Ctrl+H`, `Alt+C`, `Alt+W`, and `Alt+R` through `CODEMIRROR_EDITOR_SHORTCUTS`. The existing shared `F3` and `Shift+F3` registrations remain the navigation authority, including while a find or replace input has focus. `Enter` and `Ctrl+Alt+Enter` are also declared in that registry for replace-current and replace-all, but their `allowInInputWithin` scope limits them to the replacement field.

`codeMirrorFindReplace.ts` owns the shared `SearchQuery` construction, validity-aware metrics, and auto-navigation logic. `MarkdownRichEditor` and `TextCodeEditor` map the controlled state to that API and use CodeMirror's native `replaceNext` and `replaceAll` commands. Keep replacement operations on those native commands so CodeMirror owns selection updates and undo transactions. Search metrics must check the query validity before creating a cursor because an invalid regular expression cannot produce one.

`CodeMirrorFindReplacePopover` is intentionally non-modal so the editor remains visible and keyboard navigation remains continuous. Its Find and Replace inputs come first in DOM order to preserve the first two tab stops. The history affordance is supplied by the in-field `Search (⇅ for history)` and `Replace (⇅ for history)` placeholders; result count, match options, navigation, and close controls occupy the first row to the inputs' right. `codeMirrorFindReplaceHistory.ts` persists independent Find and Replace histories in local storage, safely tolerating unavailable or malformed storage and retaining only the ten newest unique non-empty values. Reuse dialog surface tokens for its paper, but do not use `ResponsiveFormDialog`: full-screen viewers retain their specialized focus and layout shell.
