...
describe("Browser Component", () => {
	...
	describe("Rendering", () => {
		...
		it("displays loading state while fetching files", async () => {
			...
			vi.mocked(api.listDirectory).mockImplementation(
				() =>
					new Promise((resolve) =>
						setTimeout(() => resolve(mockDirectoryListing), 100),
					),
			);
		...
		it("shows message when no connections are configured", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/No SMB connections configured/i),
				).toBeInTheDocument();
			});
		});
	...
	describe("Interaction", () => {
		...
		it("refreshes file list when refresh button clicked", async () => {
			...
			await waitFor(() => {
				expect((api.listDirectory as Mock).mock.calls.length).toBeGreaterThan(
					initialCallCount,
				);
			});
		});
	...
	describe("Error Handling", () => {
		...
		it("handles network errors", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(
						/Failed to load files. Please check your connection settings/i,
					),
				).toBeInTheDocument();
			});
		});
