...
6. **Review existing accounts**
   ...
   - prefill each active account from a previous pending username or last-seen IdP username when available; otherwise show the local username as **Unverified** and leave that row unselected until the administrator confirms or edits it
   - keep this step optional in **OIDC with local recovery**, but explain that an existing user who signs in through OIDC before being mapped may encounter a username collision or receive a separate auto-provisioned account
   - before enabling **OIDC only**, require review of the plan and explicit acknowledgement for every omitted active passwordless account
...
Before or after activation, **Existing user mappings** lists local accounts that need to retain their existing Sambee data. For each account, an administrator may enter the expected IdP username and confirm a pending mapping. On that identity's next normal OIDC login, Sambee consumes the pending mapping and permanently binds the validated `(issuer, subject)` to the selected local account. The user receives no separate mapping link, mapping page, or approval prompt. Ordinary admitted users without a pending mapping are auto-provisioned on first OIDC sign-in. Recommend **OIDC with local recovery** during migration so existing local users can continue signing in with their passwords until mapped. Initial activation creates the tested administrator identity and every selected pending mapping in one transaction, preventing an avoidable post-activation access gap.

For migrations with multiple existing accounts, **Existing user mappings** also provides a batch review table. It lists unmapped local accounts and allows inline correction and row selection. Prefer an existing pending username or the linked identity's non-authoritative last-seen IdP username. When neither exists, prefill from the local username but label it **Unverified** and leave the row unselected until the administrator confirms or edits it. Before confirmation, the backend validates every selected row for empty or duplicate usernames, existing pending or immutable mappings, inactive or deleted targets, and target conflicts. Show all validation errors in the review table and create the complete selected batch atomically only when every row is valid. Show inactive and expired accounts in a separate non-selectable section and explain that their old OIDC links will be removed during replacement and that they may be mapped after reactivation. Do not add CSV import in v1.

After activation, an Advanced provider action named **Remap all OIDC accounts** handles an IdP reinstall or migration that changed subjects even when issuer, client ID, and other provider fields remain unchanged. Supporting text states that it removes every current OIDC account link, signs out affected users, and preserves local users and their data. It runs **Connect and test** with the current configuration and then opens the shared mapping-plan review before making changes. The server derives the preview from current mappings without storing reviewed rows on the flow. It shows established and pending mapping counts; maps the tested administrator directly; and prefills every other active target from its pending username or non-authoritative last-seen IdP username. Those authoritative-source rows are initially selected and remain editable. A local-username fallback is marked **Unverified** and remains unselected until confirmed or edited. Inactive and expired targets appear separately as non-selectable accounts whose old links will be removed. In OIDC-only mode, omitting any active passwordless target requires explicit acknowledgement that the user will lose access to their existing local account. Changing issuer or client ID invokes the same review automatically; Sambee never infers identity continuity from one test account. Confirmation submits the reviewed rows and expected mapping revision and applies them in one transaction rather than deleting mappings before remapping is prepared.
...
**Activate** submits the flow ID plus the reviewed sign-in mode, admission policy, privileged group mappings, any fresh username-claim uniqueness confirmation, selected mapping-plan rows, the expected `identity_mapping_revision` when an active configuration exists, and required omitted-account acknowledgements. The test flow records the active `configuration_revision`, or records that no active configuration existed, when testing begins. Inside the activation write transaction, the backend rejects a revision/existence mismatch with **Configuration changed; connect and test again**, and rejects a submitted mapping-revision mismatch with **OIDC account mappings changed; review the updated plan and confirm again**. It revalidates every submitted row and omitted-account acknowledgement against current users and mappings. It also requires the initiating administrator to remain active and unexpired, the tested identity to be admitted and resolve to `admin`, that identity to map uniquely to the initiating administrator, and the committed result to contain at least one usable administrator. It then atomically persists the server-validated configuration, creates or verifies the tested identity mapping, creates every selected pending mapping, increments the mapping revision, and consumes the flow. Any conflict or failed write rolls back the entire transaction. **Cancel** calls the authenticated test-flow deletion endpoint, which conditionally consumes and deletes the initiating administrator's unexpired test flow and encrypted candidate configuration; closing the setup only discards the browser's flow UUID, while the server deletes the abandoned encrypted candidate when the flow expires after 30 minutes. Neither action affects active authentication. Each **Connect and test** attempt creates a new immutable flow. During one unexpired setup flow, admission and role-mapping edits recompute the preview server-side from the stored validated claims and do not require another IdP login. Editing issuer, client ID, client secret, scopes, or claim names, or allowing the test to expire, requires a new test. A later admission or role-policy change while OIDC-only remains active still requires a fresh test sign-in. Login flows retain secret, hashed one-time grants.

Treat issuer and client ID as the identity-namespace boundary. Changing either field always requires replacement; do not infer continuity by comparing one tested identity. Administrators may also invoke **Remap all OIDC accounts** without changing provider fields when an IdP reinstall or migration regenerated subjects. Replacement requires a fresh interactive test and the shared mapping-plan review before any mapping changes. The final request carries the reviewed rows and expected `identity_mapping_revision`; the flow does not persist them. The confirmation transaction verifies both revisions and repeats every administrator, row, acknowledgement, uniqueness, and usable-administrator check required by activation; persists the tested provider update; deletes the old established and pending mappings; maps the tested administrator; creates the complete reviewed set of pending mappings for other selected targets; increments affected users' `token_version`; increments both revisions, including for same-configuration replacement; consumes the flow; and writes the audit event. Any failed check or write rolls back the entire transaction. It never reconnects another user by username or email without an administrator-reviewed pending mapping. Recommend **OIDC with local recovery** during replacement. A correctly migrated reinstall that preserves issuer, client ID, and every subject needs no replacement and retains all mappings.
...
- `GET /api/admin/auth/oidc/test-flows/{flow_id}` returns the safe claim preview and, when local accounts require migration or replacement, a server-derived mapping-plan preview plus the current `identity_mapping_revision` without consuming the flow or storing reviewed rows. Rows include target state, prefill source, selection default, and whether omission requires acknowledgement. Return `404` when the flow does not exist or belongs to another administrator
- `DELETE /api/admin/auth/oidc/test-flows/{flow_id}` conditionally consumes and deletes an unexpired purpose-`test` flow belonging to the current administrator, including its encrypted candidate, and returns `204`; return `404` when it does not exist, is already terminal, or belongs to another administrator
- `POST /api/admin/auth/oidc/replace-identity-namespace` is the internal endpoint for the UI's **Remap all OIDC accounts** action. It requires and consumes a successful administrator-bound test flow, accepts reviewed `{target_user_id, expected_username}` rows, the expected mapping revision, and omitted-account acknowledgements, and performs the same guarded atomic replacement transaction used by issuer or client-ID changes
...
`oidc_mapping_review_stale` is admin-only. On receipt, the frontend refetches the server-derived plan, preserves submitted edits for unchanged target users, and highlights rows whose source data or eligibility changed. Other admin validation and mapping APIs use the same codes where applicable and may add safe field and row context. Do not expose provider payloads, claim values, subjects, secrets, or internal exception text.
...
- authenticated test-flow preview and explicit cancellation enforce purpose, status, initiating-administrator binding, terminal-state handling, and encrypted-candidate deletion
...
- server-derived mapping-plan previews persist no reviewed rows and identify each prefill as pending, last-seen IdP username, or unverified local username
...
- initial activation may create reviewed pending mappings in the same transaction as the tested administrator identity; OIDC-only rejects missing acknowledgement for any omitted active passwordless account
...
- stale mapping-plan recovery refetches current rows, preserves edits for unchanged targets, and identifies changed eligibility or prefill sources
...
- expected IdP username prefers an existing pending or last-seen IdP value; a local-username fallback is labeled **Unverified**, remains unselected, and requires explicit confirmation or editing
...
- the same stateless mapping-plan review is available during initial activation and replacement, permits inline correction and omission, and requires explicit acknowledgement for every omitted active passwordless target in OIDC-only mode
...
- a stale-plan response refetches current rows, preserves edits for unchanged targets, highlights changed rows, and never silently resubmits
...
- initial activation atomically maps the tested administrator and creates every selected pending mapping from the shared plan; omitted active passwordless users require explicit acknowledgement in OIDC-only mode
...
- OIDC-only replacement never commits before omitted active passwordless targets are explicitly acknowledged, and selected targets receive pending mappings in the replacement transaction
