...
- create the configured bootstrap administrator only when both the user table is empty and no database authentication configuration exists; once either exists, startup never creates a missing configured administrator or changes an existing user's role, activity, expiry, or password, and instead reports a missing usable administrator through typed authentication health and actionable redacted logs
