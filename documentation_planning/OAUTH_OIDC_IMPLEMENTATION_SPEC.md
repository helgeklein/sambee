...
- **Local password login:** authentication with an existing Sambee username and password when the configured sign-in mode permits it.
...
4. **Access policy and roles**
   - choose the intended OIDC mode: **OIDC or password** (default) or **OIDC only**
...
5. **Review existing accounts**
   ...
   - keep this step optional in **OIDC or password**, but emphasize omitted active passwordless accounts and explain that any existing user who signs in through OIDC before being mapped may encounter a username collision or receive a separate auto-provisioned account
...
- In **OIDC or password** mode, show **Sign in with {provider display name}** as the primary action and the Sambee username/password fields on the same `/login` page, separated clearly with **or**. Label the local action **Sign in with password** so users can distinguish Sambee credentials from identity-provider credentials.
- In **OIDC only** mode, `/login` automatically starts one top-level OIDC authorization attempt while preserving the sanitized return path. Do not render a login chooser, provider button, or password form during the normal path.
...
- In **OIDC only**, record the automatic attempt in `sessionStorage` before navigation. After an OIDC failure, configuration failure, or rate limit, suppress another automatic redirect and render only a minimal local error state with **Try again**. Explicit logout also suppresses automatic reauthentication and renders a minimal signed-out state with **Sign in again**. Clear the suppression marker after successful login or when the user explicitly retries.
- Do not add `/login/local`; `/login` is the only interactive login route. The password API is unavailable in **OIDC only** even though a failure or signed-out state may be rendered there.
...
Password login is available on `/login` only in **Password only** and **OIDC or password** modes and only to accounts with an existing local password. Both modes use the same backend password endpoint and the same IP and normalized-username rate-limit buckets; neither the limiter nor its response distinguishes the current sign-in mode. Password login never bypasses normal password verification, activity, expiry, or token-version checks.
...
- if the calculated role would leave no usable administrator, preserve the stored role and mapping, increment `token_version`, emit an audit event that records whether a local password exists, fail the login with `oidc_last_administrator_role_conflict` or its `oidc_last_administrator_role_conflict_no_password` variant as applicable, and issue no Sambee JWT
...
- create the configured bootstrap administrator only when both the user table is empty and no database authentication configuration exists; once either exists, startup never creates a missing configured administrator or changes an existing user's role, activity, expiry, or password, and instead adds `no_active_administrator` to typed authentication health and emits actionable redacted logs
- never reactivate, unexpire, promote, or delete an account automatically
...
| `sign_in_mode` | enum | `password_only`, `oidc_or_password`, or `oidc_only` |
...
- Return administrator authentication health as `status` (`healthy` or `unhealthy`) plus an ordered `reasons` array. Freeze the reason enum as `oidc_secret_key_missing`, `oidc_secret_key_invalid`, `oidc_secret_decryption_failed`, `public_url_missing`, `public_url_invalid`, and `no_active_administrator`. An empty array is the only healthy result. Return simultaneous failures in that listed order; never include exception text or unknown codes.
...
- Keep local password login available when the configured sign-in mode permits it; OIDC key failure does not silently change persisted login settings.
...
The OIDC lifetime is not an administrator setting in v1. In OIDC-only mode, an unauthenticated visit to `/login`, including one caused by session expiration, automatically starts OIDC reauthentication once while preserving the safe return path; an active IdP session normally makes this redirect transparent. Set an automatic-attempt marker in `sessionStorage` before top-level navigation to prevent redirect loops, and clear it after successful login. On provider, configuration, callback, or rate-limit failure, retain the marker and render a minimal error state with **Try again** and a generic message that sign-in is temporarily unavailable. Explicit logout sets a separate suppression marker, clears any stale attempt marker, and renders a minimal signed-out state with **Sign in again** instead of immediately returning the user to an active IdP session. Either explicit action clears the applicable marker immediately before starting a new top-level OIDC attempt. OIDC-only intentionally provides no local-password fallback during an IdP outage. OIDC-or-password mode returns to the combined `/login` page with the provider action and local password form still available.
...
`sign_in_mode` is the sole public source of truth. The frontend derives available methods from it. Do not add an `oidc` value to the legacy backend bootstrap enum; it cannot represent the database-owned OIDC modes correctly.
...
- Enforce bounded application-level rate limits by default: authorization starts at 20 requests per source IP per 5 minutes, callbacks at 60 per source IP per 5 minutes, exchanges at 30 per source IP per 5 minutes, and password login at 10 attempts per source IP per 5 minutes plus 10 attempts per normalized username per 15 minutes. Authorization, callback, exchange, and the shared password endpoint use independent buckets; the normalized-username password bucket is an additional check rather than a separate endpoint bucket. The password limits apply in Password-only and OIDC-or-password modes.
- Implement each limit as a token bucket whose capacity is the stated request count and whose tokens refill continuously across the stated duration. Rejected requests do not consume tokens. Calculate `Retry-After` as the ceiling of the monotonic-clock duration until one token is available. Refill, allow/reject, consumption, last-used update, and `Retry-After` calculation for one request must be atomic under a process-local synchronization primitive that is never held across I/O.
- Treat the password IP and username buckets as one atomic admission decision under that synchronization primitive. Refill both and admit only when both contain a token. If either is empty, consume neither, update both LRU positions, and return the maximum ceiling wait until both can admit a request. Otherwise consume one token from each and update both LRU positions before releasing the lock.
- Use bounded in-memory TTL/LRU maps for the supported single-application-process deployment, with separate capacities of 10,000 source-IP keys and 10,000 username keys. Remove fully refilled inactive entries before evicting the least-recently-used entry at capacity; capacity is handled only through eviction, never blanket rejection of unseen keys. A restart may clear baseline limits. Multi-instance deployments require a shared limiter with equivalent atomic semantics before horizontal scaling.
- Build the password username-bucket key by trimming surrounding whitespace while preserving case and Unicode code points, then hashing the complete normalized UTF-8 value with SHA-256. Store only that fixed-size hash, and do not change the exact submitted username used for password lookup. Enforce a 64 KiB password-form body limit at the ASGI boundary before form parsing; accept the exact limit, reject one byte over with a generic `413`, and never log or reflect body content. Do not merge long usernames into a shared limiter key.
- After the transport-level body guard, resolve password-endpoint availability before form parsing or limiter access. With no database authentication configuration, permit only legacy `password`; with a database configuration, permit only `password_only` and `oidc_or_password`. Legacy `none` and `oidc_only` return fixed `404` without parsing credentials or changing limiter state. This separate availability gate does not pass the sign-in mode into the limiter. The 64 KiB guard intentionally remains an earlier transport-safety exception and may return generic `413` in any mode.
- Derive source IP from the direct socket peer by default. Every repository-maintained Uvicorn launch path must pass `--no-proxy-headers`, making Sambee the sole forwarding-header authority. `SAMBEE_TRUSTED_PROXY_CIDRS` is an optional comma-separated environment variable containing validated IP addresses or CIDR ranges and defaults to empty. Only when the direct peer matches a configured range, parse `X-Forwarded-For` strictly from right to left, skip trusted addresses, and select the first untrusted address. Use the direct peer when the chain is malformed or contains no untrusted address. Support IPv4 and IPv6; never accept hostnames, partial addresses, or arbitrary client-supplied forwarding headers as rate-limit keys.
...
- update `Login` for OIDC-or-password, OIDC-only, password-only, and `none` states
...
- `app/core/auth_methods.py`: preserve bootstrap `password`/`none` behavior while database configuration represents OIDC availability
...
- activation of either OIDC mode creates reviewed pending mappings in the same transaction as the tested administrator identity; OIDC-only requires acknowledgement for every omitted active account, and OIDC-or-password mode emphasizes omitted passwordless accounts
...
- Password-only, OIDC-or-password, OIDC-only, and `none` modes each preserve expected behavior
...
- OIDC-or-password renders the provider action, clear separator, and Sambee username/password form together on `/login`; both methods preserve the same valid return route
- no `/login/local` route is registered or accepted in any mode
- the OIDC button uses the backend authorization path and preserves a valid return route
- OIDC-only performs one top-level automatic authorization navigation without rendering a chooser, provider button, or password form
- OIDC-only records the attempt before navigation; provider, configuration, callback, and rate-limit failures suppress another automatic navigation and render only the minimal **Try again** state
- explicit logout in OIDC-only sets separate suppression state and renders only **Sign in again**; explicit retry clears the applicable state before navigation, and successful login clears all suppression state
...
- the same stateless mapping-plan review is available during initial OIDC activation and replacement, permits inline correction and omission, requires acknowledgement for every omitted active target in OIDC-only mode, and emphasizes omitted passwordless targets in OIDC-or-password mode
...
- a passwordless last administrator receives an explicit instruction to restore the IdP group rather than attempt unavailable password login
- disabled local account is denied despite valid provider authentication
- local password login remains available on the combined login page during a provider outage in OIDC-or-password mode
- a signed-out OIDC-only visit starts top-level OIDC authorization without first showing login controls
...
- explicit logout in OIDC-only does not immediately reauthenticate through an active IdP session and requires **Sign in again**
...
- `/login` shows both the provider action and local password form in OIDC-or-password mode; no `/login/local` route exists, and password login retains its rate limiting
...
- initial account migration and **Remap all OIDC accounts**, including the shared stateless pre-commit mapping-plan review, affected-user sign-out, inactive-user disposition, omitted-user access consequences, and OIDC-or-password continuity
...
- `SAMBEE_PUBLIC_URL`, the derived callback URI, combined-login behavior, and transparent OIDC-only behavior
...
**Recommended:** yes. Use OIDC-or-password mode by default so local-password accounts remain available, but permit OIDC-only after the current administrator successfully maps and tests their identity. Treat IdP availability as a required dependency in OIDC-only mode.

Use one `/login` page in OIDC-or-password mode so both the provider action and clearly labeled Sambee password form are immediately available. In OIDC-only mode, start OIDC automatically during the normal path and show only minimal retry or signed-out UI after failure or explicit logout.

**Decision:** use **OIDC or password** mode by default, with canonical value `oidc_or_password`. Show the provider action and local password form together on `/login`; do not add `/login/local`. In **OIDC only**, automatically start one OIDC attempt without rendering login choices, suppress redirect loops after failure or explicit logout, and show only minimal retry or signed-out UI. Permit either OIDC mode only after the current administrator maps and successfully tests an identity that is admitted and resolves to `admin`; OIDC-only has no local fallback when the IdP is unavailable. Treat Password-only as a direct mode action outside OIDC setup. Its API transaction verifies the administrator-acknowledged active-and-unexpired passwordless-account count, requires renewed review if the count changed, and refuses the UI action unless an active unexpired administrator has a local password. Use `sambee auth set-mode password-only` for administrative recovery from bad configuration or lost access; it reports usable local-password administrators and affected passwordless accounts, refuses a lockout by default, rechecks both counts after confirmation, permits an explicitly warned CLI-only `--force` for deliberate IdP containment, and never resets or bypasses credentials.
