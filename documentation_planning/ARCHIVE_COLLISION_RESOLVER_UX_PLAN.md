...
- Present source and target identities as separately labelled, full-width values. Show the target parent directory, but do not show a source path.
...
- Treat a destination that exists at any point as an overwrite conflict, never as a member error.
...
3. Full-width one-line fields show the source name, target name, and target directory. The target-name field is read-only unless Rename is selected. A separate static comparison presents size and modification time when useful.
...
## Collision And Member-Error Boundary

An existing target is a conflict whether it is detected during preflight or appears after the last target inspection. This is a domain rule shared by direct backend extraction, direct local extraction, and every relay topology; the frontend must not infer it from an error message.

At every exclusive target creation or write boundary, adapters must map their native existence signal (`FileExistsError` or `AlreadyExists`) to the existing-files decision payload. They re-read target metadata where their storage API permits it, then pause through the same coordinator and decision contract used for a preflight collision. A target that disappears before metadata is read is an ordinary member write failure, because there is no longer a conflict to resolve.

All other source-read, integrity, permission, capacity, and write failures remain member errors. An existence race must not create a partial-member checkpoint unless bytes have actually been written. A late collision after a `replace_older` decision is evaluated against fresh metadata: it skips an equal, newer, or incomparable target and retries replacement only for an older target. It reopens the normal overwrite resolver only when a bounded replacement retry encounters another unresolved race.

### Member Error Recovery

Member error recovery is a compact, distinct state in the existing extraction workflow, not another overwrite form.

- Show one severity-appropriate alert with the actionable error message.
- Show `Archive member` and `Target path` in one compact definition list. Each path stays on one horizontally scrollable line; do not use large read-only form fields or side-by-side settings surfaces.
- Show a short partial-output note only when retrying must replace an incomplete target.
- Use three direct footer actions: `Cancel extraction`, `Skip member`, and the primary `Retry member`. Do not require a radio selection followed by `Continue`.
- Initially focus `Retry member` when it is safe and enabled. Native button activation handles Enter; Escape and Cancel use the owner cancellation request.
- Keep the protocol action name `ignore` if needed for compatibility, but label it `Skip member` in the UI.
...
Clear the checkbox whenever the resolution changes. A checked control requests an all-remaining action from the owner, rather than promising frontend-managed behavior.
...
- Existing target item kind, display name, target parent-directory path, optional size, and optional modification time.
...
- Keep source name, target name, and target directory as separate full-width one-line horizontally scrollable fields at every breakpoint. Use desktop comparison columns only for compact metadata and labelled stacked metadata on narrow screens. Title, prompt, identity fields, radios, bulk control, and both actions must remain reachable in a short viewport through vertical body scrolling. Selecting Rename must not shift this layout.
...
5. Add explicit late-existence classification at every backend, direct-local, and relay target-write boundary; retain member recovery only for non-collision failures.
6. Replace member-error radio/Continue flow with direct Retry and Skip actions and compact path context.
7. Remove archive-only multi-conflict list UI, duplicate cancellation actions, and invalid block-in-paragraph comparison markup.
8. Verify responsive Drawer/Dialog behavior, operation recovery, and truthful progress labels across all adapters.
9. When implementation begins, document the reusable component and adapter contract in the developer architecture documentation.
...
- `Overwrite only older` is an explicit operation-wide policy without a redundant bulk checkbox.
...
- A target created after preflight, including after `Overwrite only older`, enters the existing-files decision state for every execution topology rather than member recovery.
- Genuine source, integrity, permission, capacity, and partial-write failures remain member errors with direct Retry and Skip actions.
...
- The identity summary lists source name, target name, and target directory only; the directory field excludes the target filename. The compact metadata comparison lists useful known size and modification timestamp values for source and existing target.
...
- A bulk choice affects only future conflicts in the same operation and only when the owner exposes an authoritative all-remaining action; changing resolution always clears the bulk choice.
