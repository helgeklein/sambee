...
pub async fn require_auth(State(state): State<Arc<AppState>>, request: Request<axum::body::Body>, next: Next) -> Response {
    match extract_auth_credentials(&request) {
        Ok((origin, hmac_value, timestamp_str)) => match validate_hmac(&state, &origin, &hmac_value, &timestamp_str) {
...
pub async fn require_auth_or_query(State(state): State<Arc<AppState>>, request: Request<axum::body::Body>, next: Next) -> Response {
    ...
    if let Ok((origin, hmac_value, timestamp_str)) = extract_auth_credentials(&request) {
        return match validate_hmac(&state, &origin, &hmac_value, &timestamp_str) {
            Ok(()) => next.run(request).await,
            Err(e) => e.into_response(),
        };
    }
    ...
    match validate_hmac(&state, &origin, &hmac_value, &timestamp_str) {
        Ok(()) => next.run(request).await,
        Err(e) => e.into_response(),
    }
}
...
pub(super) fn validate_hmac_public(state: &AppState, origin: &str, hmac_value: &str, timestamp_str: &str) -> Result<(), ApiError> {
    validate_hmac(state, origin, hmac_value, timestamp_str)
}

/// Validate HMAC credentials against the stored secret for an origin.
fn validate_hmac(state: &AppState, origin: &str, hmac_value: &str, timestamp_str: &str) -> Result<(), ApiError> {
    // Get shared secret for this origin from pairing state
    let secret = state.pairing.get_secret_for_origin(origin).ok_or_else(|| {
        warn!("Auth rejected: no pairing found for origin {origin}");
    ...
    if diff > MAX_TIMESTAMP_SKEW {
        warn!("Auth rejected: timestamp skew {diff}s exceeds {MAX_TIMESTAMP_SKEW}s");
    ...
    if expected != hmac_value {
        warn!("Auth rejected: HMAC mismatch for origin {origin}");
